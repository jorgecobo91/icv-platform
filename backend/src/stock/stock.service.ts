...
export class StockService {
  ...
  async importStock(file: Express.Multer.File) {
    ...
    return {
      message: 'Stock importado correctamente',
  ...
  async obtenerAlmacenesCruzados() {

    const stock = await this.stockRepository.find({
      relations: ['material'],
    });

    const cruzados = stock.filter((s) => {

      if (s.tipo === '001' && s.almacen !== '0001') return true;
      if (s.tipo === '003' && s.almacen !== '0003') return true;

      return false;

    });

    return cruzados.map((s) => ({
